...
describe("✅ WORKING CONTRACT TEST - YOUR EXACT DATA", function () {
  ...
  describe("✅ REAL CONTRACT FUNCTION CALLS", function () {
    ...
    it("🎯 ACTUALLY CALLS createAnonymousRecord with your EXACT World ID data", async function () {
      ...
      // FOLLOWING EXACT COUNTER PATTERN - Encrypt each value separately
      const [encryptedProof] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([Encryptable.uint256(proofValue)] as const)
      );
      const [encryptedNullifier] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([Encryptable.uint256(nullifierValue)] as const)
      );
      const [encryptedMerkleRoot] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([Encryptable.uint256(merkleRootValue)] as const)
      );
      const [encryptedVerificationType] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([Encryptable.uint256(verificationTypeValue)] as const)
      );
      const [encryptedCredentialType] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([Encryptable.uint256(credentialTypeValue)] as const)
      );

      console.log("  ✅ Encryption successful!");
      console.log("  📊 Encrypted 5 individual parameters");

      console.log("\n📤 CALLING CONTRACT: createAnonymousRecord...");

      // **ACTUALLY CALL THE CONTRACT** with your exact data - FOLLOWING COUNTER PATTERN
      const tx = await anonymityTiers.connect(user1).createAnonymousRecord(
        encryptedProof, // proof
        encryptedNullifier, // nullifierHash
        encryptedMerkleRoot, // merkleRoot
        encryptedVerificationType, // verificationType
    ...
    it("🎯 ACTUALLY CALLS createIdentityRecord with your EXACT Self data", async function () {
      ...
      // FOLLOWING COUNTER PATTERN - Encrypt each value separately
      const [encryptedWorldProof] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([Encryptable.uint256(worldProofValue)] as const)
      );
      const [encryptedWorldNullifier] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([Encryptable.uint256(worldNullifierValue)] as const)
      );
      const [encryptedWorldMerkleRoot] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([Encryptable.uint256(worldMerkleRootValue)] as const)
      );
      const [encryptedWorldVerificationType] =
        await hre.cofhe.expectResultSuccess(
          cofhejs.encrypt([
            Encryptable.uint256(worldVerificationTypeValue),
          ] as const)
        );
      const [encryptedWorldCredentialType] =
        await hre.cofhe.expectResultSuccess(
          cofhejs.encrypt([
            Encryptable.uint256(worldCredentialTypeValue),
          ] as const)
        );
      const [encryptedSignedMessage] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([Encryptable.uint256(signedMessageValue)] as const)
      );
      const [encryptedUserId] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([Encryptable.uint256(userIdValue)] as const)
      );
      const [encryptedSelfNullifier] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([Encryptable.uint256(selfNullifierValue)] as const)
      );
      const [encryptedNameHash] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([Encryptable.uint256(nameHashValue)] as const)
      );
      const [encryptedNationalityHash] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([Encryptable.uint256(nationalityHashValue)] as const)
      );
      const [encryptedAge] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([
      ...
      console.log("  ✅ Encryption successful!");
      console.log("  📊 Encrypted 11 individual parameters");
      ...
      const tx = await anonymityTiers.connect(user1).createIdentityRecord(
        encryptedWorldProof, // worldId proof
        encryptedWorldNullifier, // worldId nullifierHash
        encryptedWorldMerkleRoot, // worldId merkleRoot
        encryptedWorldVerificationType, // worldId verificationType
        encryptedWorldCredentialType, // worldId credentialType
        encryptedSignedMessage, // signedMessage
        encryptedUserId, // self userId
        encryptedSelfNullifier, // self nullifier
        encryptedNameHash, // nameHash
        encryptedNationalityHash, // nationalityHash
        encryptedAge // age
      );
    ...
    it("🎯 ACTUALLY TESTS the complete anonymity tier progression", async function () {
      ...
      const [encryptedAge] = await hre.cofhe.expectResultSuccess(
        cofhejs.encrypt([Encryptable.uint32(BigInt(derivedData.age))] as const)
      );
